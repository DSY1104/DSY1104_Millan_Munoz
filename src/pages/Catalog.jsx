...
export default function CatalogPage() {
   ...
   useEffect(() => {
      const fetchData = async () => {
         try {
            setLoading(true);
            const [productsData, categoriesData] = await Promise.all([getActiveProducts(), getAllCategories()]);
   ...
   const uniqueBrands = useMemo(() => {
      const set = new Set();
      products.forEach((p) => {
         if (p.marca && typeof p.marca === "string" && p.marca.trim() !== "") set.add(p.marca.trim());
      });
   ...
   const filteredProducts = useMemo(() => {
      ...
      if (q) {
         filtered = filtered.filter((p) => normalizeText(p.nombre).includes(q) || normalizeText(p.code).includes(q));
      }
      if (selectedCategory) {
         filtered = filtered.filter(
            (p) =>
               ...
               (p.categoria?.nombreCategoria &&
                  normalizeText(p.categoria.nombreCategoria).includes(normalizeText(selectedCategory)))
         );
      }
      if (selectedBrand) {
      ...
      if (selectedRating !== null) {
         filtered = filtered.filter(
            (p) => typeof p.rating === "number" && p.rating >= selectedRating && p.rating < selectedRating + 1
         );
      }
      if (minPrice !== null) {
      ...
      return filtered;
   }, [products, search, sortPrice, sortRating, selectedCategory, selectedBrand, selectedRating, minPrice, maxPrice]);
   ...
   useEffect(() => {
      setCurrentPage(1);
   }, [search, sortPrice, sortRating, selectedCategory, selectedBrand, selectedRating, minPrice, maxPrice]);
         ...
         <main className="catalog__main-content">
            <header className="catalog-header">
               ...
               <p className="catalog-subtitle">
                  Explora nuestra amplia gama de productos y encuentra lo que necesitas para llevar tu experiencia gamer
                  al siguiente nivel.
               </p>
            </header>

            {loading && <div style={{ textAlign: "center", padding: "3rem", color: "#aaa" }}>Cargando catálogo...</div>}

            {error && <div style={{ textAlign: "center", padding: "3rem", color: "#ff6b6b" }}>{error}</div>}
                        ...
                        <div className="catalog__product-grid" id="product-list">
                           {paginatedProducts.length === 0 ? (
                              ...
                              >
                                 ...
                                 >
                                    Prueba ajustando los filtros, revisa la ortografía o explora otras categorías.
                                    <br />
                              ...
                              paginatedProducts.map((product) => (
                                 <ProductCard key={product.idProducto || product.id || product.code} {...product} />
                              ))
                           )}
                        </div>
                        {/* Pagination Controls */}

...
function renderPaginationControls() {
  ...
  //     setTimeout(() => {
  //       const newProductStart = document.querySelector(
  //         `#product-list .product-card:nth-child(${
  //           (currentPage - 1) * PRODUCTS_PER_PAGE + 1
  //         })`
  //       );
  //       if (newProductStart) {
  //         newProductStart.scrollIntoView({
  //           behavior: "smooth",
  //           block: "start",
  //         });
...
function updateQueryString() {
  ...
  if (sortPrice && sortPrice.value !== "none")
    params.set("sortPrice", sortPrice.value);
  if (sortRating && sortRating.value !== "none")
    params.set("sortRating", sortRating.value);
...
function restoreFromQueryString() {
  ...
  // Note: Brand and rating filters are handled by their respective components

  // Always render after restoring state, even if nothing changed
  renderFiltered();
}
  ...
  if (filteredProducts.length === 0) {
    container.innerHTML = `
      <div class="no-products-message fade-in" style="display: flex; flex-direction: column; align-items: center; justify-content: center; width: 100%; min-height: 220px; text-align: center;">
        <img src="../../assets/image/icon/status-error.svg" alt="Sin resultados" style="width: 80px; margin-bottom: 1em; opacity: 0.7;" />
        <span class="no-products-title" style="color: var(--accent-blue); font-size: 1.5em;">No encontramos productos</span>
        <span class="no-products-sub" style="color: #888; font-size: 1.1em; font-family: Roboto, sans-serif">Prueba ajustando los filtros, revisa la ortografía o explora otras categorías.<br>¡Siempre hay algo nuevo por descubrir!</span>
      </div>
    `;
